    ...
    private void initListView(ItemDto dto) {
        ...
        listView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                ItemDto dto = commandAdaptor.getDataModel().get(position);
                goDemoView(dto.getActivtyClass());
            }
        });
        commandAdaptor.notifyDataSetChanged();
    }

    private void goDemoView(Class clazz) {
        if(clazz!=null && clazz.getName().contains("Activity")){
